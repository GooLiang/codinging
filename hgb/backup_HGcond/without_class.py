            ...
            if args.method == 'random' or args.method == 'herding' or args.method == 'kcenter':
                ...
                for key_A, key_B in key_counter.items():
                    ...
                    for key in key_B:
                        ...
                        ppr_sum[key_A] += ppr[key_A][key][score_train_idx] ##不同metapath直接相加，这里可以考虑优化
                        # ppr[key_A][key] = None
                    ppr_sum[key_A] = torch.sum(ppr_sum[key_A], dim = 0)
                if args.dataset == 'DBLP':
                    candidate = {}
                    candidate[tgt_type] = score_train_idx
                    ...
                    for key, value in ppr_sum.items():
                        ...
                        if reduce_nodes == 0:
                            _, candidate[key] = torch.topk(ppr_sum[key], k = int(0.1 * node_type_nodes[key]), largest = True)
                        else:
                            _, candidate[key] = torch.topk(ppr_sum[key], k = reduce_nodes, largest = True)
                    x = 1
                    new_adjs = {}
