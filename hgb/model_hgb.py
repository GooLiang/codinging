...
#     def __init__(self, feats_keys, extra_feat_keys, label_feat_keys, data_size, data_size_extra, in_feats, r_len, tgt_type, input_dropout, dropout, hidden, out_feats, n_layers_2, att_drop, sum_metapath, bns):
...
#         self.dropout = nn.Dropout(dropout)
#         self.prelu = nn.PReLU()
#         self.sum_metapath = sum_metapath
#         ####self.r_ensemble_w = []   ###  nn.ParameterList() 二者之间的区别:对打印模型有影响，对参数量没有影响
...
#         # ########### new for ensemble ##########
#         # self.mid_project = nn.ParameterList()
#         # for _ in range(r_len):
#         #     self.mid_project.append(FeedForwardNet(
#         #         hidden, hidden, hidden, n_layer_mid, dropout
#         #     ))
#         # self.mid_project_out = FeedForwardNet(
#         #         hidden, hidden, out_feats, n_layer_mid_out, dropout
#         #     )
#         # ########### new for ensemble ##########
...
#         #self.epsilon = torch.FloatTensor([1e-12]).to('cuda:0')
#         self.gamma = nn.Parameter(torch.tensor([0.]))
#         if not self.sum_metapath:
#             self.concat_project_layer = nn.Linear((num_channels) * hidden, hidden)
...
#                     nn.init.zeros_(layer.bias)
#         # label_fc_layers = [
#         #     [nn.Linear(hidden, hidden, bias=not True)] + add_nonlinear_layers(hidden, dropout, True)
#         #     for _ in range(1)]
#         # self.label_fc = nn.Sequential(*(
#         #     [nn.Linear(out_feats, hidden, bias=not True)] + add_nonlinear_layers(hidden, dropout, True) \
#         #     + [ele for li in label_fc_layers for ele in li] + [nn.Linear(hidden, out_feats, bias=True)]))
...
#             global_vector = torch.softmax(torch.sigmoid(torch.matmul(feats, embed_weight)).squeeze(2), dim=-1)
#             output_r = 0
#             if self.sum_metapath:
#                 for i in range(feats.shape[1]):
#                     output_r = output_r + feats[:,i,:].mul(self.att_dropout(global_vector[:, i].unsqueeze(1)))
#             if not self.sum_metapath:
#                 output_r = []
#                 for i in range(feats.shape[1]):
#                     output_r.append(feats[:,i,:].mul(self.att_dropout(global_vector[:, i].unsqueeze(1))))
#                 output_r = torch.stack(output_r, dim = 1)
#                 output_r = self.concat_project_layer(output_r.reshape(feats.shape[0], -1))

#             new_feats_r.append(output_r)
#         #new_feats = new_feats_r[0]

#         # ##############  new for ensemble#########
#         # new_feats_r_mid = []
#         # for r, mid_project in zip(new_feats_r, self.mid_project):
#         #     #new_feats_r_mid.append(self.dropout(self.prelu(mid_project(r))))
#         #     new_feats_r_mid.append((mid_project(r)))
#         # new_feats_r = new_feats_r_mid
#         # ##############  new for ensemble#########
        
#         # # # ################共享w################################################################
#         h = torch.stack(new_feats_r, dim=1)
#         #h = self.dropout(self.prelu(h))
...
#         ###########################################################################################
#         #output_r_final = new_feats_r[0]

#         #output_r_final = self.gamma * output_r_final
#         # ##############  new for ensemble######### 
#         # output = self.mid_project_out(output_r_final)
#         # ##############  new for ensemble#########
#         #output = self.dropout(self.prelu(output_r_final))
#         output = self.lr_output(output_r_final)
#         #output = output + self.label_fc(self.label_drop(label_emb))
#         #output = output / (torch.max(torch.norm(output, dim=1, keepdim=True), self.epsilon))
...
class GlobalMetaAggregator(nn.Module):
    def __init__(self, feats_keys, extra_feat_keys, label_feat_keys, data_size, data_size_extra, in_feats, r_len, tgt_type, input_dropout, dropout, hidden, out_feats, n_layers_2, att_drop, sum_metapath, bns):
        ...
        for layer in self.layers:
            if isinstance(layer, Conv1d1x1):
                layer.reset_parameters()
        #self.epsilon = torch.FloatTensor([1e-12]).to('cuda:1')
        self.gamma = nn.Parameter(torch.tensor([0.]))
    ...
    def forward(self, raw_feats_ensemble, extra_features_buffer, r_label_dict):
        ...
        for r, embedding, r_extra_features, extra_embedding, r_lable_features, labels_embeding in zip(raw_feats_ensemble, self.r_embedding, extra_features_buffer, self.r_extra_embedding, r_label_dict, self.r_label_embedding):
            ...
            for feats, embed_weight in zip(extra_feats_list, self.embed_weight_r_extra):
                ...
                output_r = 0
                for i in range(feats.shape[1]):
                    output_r = output_r + feats[:,i,:].mul(global_vector[:, i].unsqueeze(1))
                extra_feats.append(output_r)
            #########enhance the extra features from de-redundancy scheme#######
        ...
        for feats, embed_weight in zip(feats_ensemble, self.embed_weight_r):
            ...
            output_r = 0
            for i in range(feats.shape[1]):
                output_r = output_r + feats[:,i,:].mul(self.dropout(global_vector[:, i].unsqueeze(1)))
            new_feats_r.append(output_r)
        ...
        output_r_final = 0
        for i, hidden in enumerate(new_feats_r):
            output_r_final = output_r_final + hidden.mul(self.dropout(global_vector[:, i].unsqueeze(1)))
